# TechnologiesOfConversation

```
Description:
Open-source, open-ended, hyper-forked poetic conversation
```

## How to teach what you really want to learn?
  ```Monica Hoff```
  ### Teach and learn beyond the possible
  ###  Accept that the whole process can be a collective learning experience by bringing people together right from the beginning.
  ### Learning has its own time.
  ### Accept your ignorance and cultivate unlearning.
  ### Appreciate what you know.
  ### Be comfortable with not knowing.
  ### Question if the subject is of common interest.
  ### Be open to discovery.
  ### Try to apply what you already know to this opportunity of learning.
  ### Go beyond correct answers, look for responsible answers.

## Whose side are you on when you choose to teach and learn?
  ### Lose the School.
  ### Lose the -isms.
  ### Forget the outcomes.
  ### Education can occur anywhere.
  ### Within the learning context, question your own reality and provoke others to do the same.
  ### Privilege lurks everywhere, expose it and challenge it.
  
## What does being conscuiouss mean?
  ### 

## What is critical conscuioussness that promotes equility, diversity, and intersectionality?
  ### It is a percepective
  ### It requires you to know how to ask questions.
  ### 

## How to promote critical consciousness?
  ### Try to understand the nature of technology
  ### Try to understand the nature need
  ### Diversity Matters.
  ### 

## How can we create and repurpose technologies that promote critical consciousness?
  ### Who made it, and for whom

## What do lose when you create a pedagogy?

## What do lose when you create a model?

## What is the difference between mastering something, and controlling it?

## How to not be controlled by something you seek to master?
  ### Commit yourself to (mastering) (enjoying the process of (an iterant) learning of a skill), while at the same time distrust the skill.
  ### Whenever possible, return to what you think you know 
  ### Revisit the poetic

## What do you lose when you learn to control?

## How to make this life pleasurable, joyfull and sustainable?
  ### Watch the sunset.
  ### Wander, linger and delay.
  ### Moments of procrastination can be extremely important.
  



```diversity matters```
```














